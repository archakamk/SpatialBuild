...
OPENSPLAT_DIR="$WORKSPACE/reconstruction_pipeline/OpenSplat"
...
# Check the known hardcoded binary path first
HARDCODED_BIN="$SCRIPT_DIR/OpenSplat/build/OpenSplat/build/opensplat"
if [ -f "$HARDCODED_BIN" ]; then
    OPENSPLAT_BIN="$HARDCODED_BIN"
    info "OpenSplat already built: $OPENSPLAT_BIN"
else
    info "OpenSplat binary not found at $HARDCODED_BIN"
